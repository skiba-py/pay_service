...
async def get_payment_by_id(payment_id: str, session: AsyncSession = Depends(get_session)):
    payment = await payment_service.get_payment_by_id(payment_id=payment_id, session=session)
    if not payment:
        return Response(status_code=HTTP_404_NOT_FOUND)
    return payment
...
async def create_payment(payment_data: PaymentSchema, session: AsyncSession = Depends(get_session)):
    try:
