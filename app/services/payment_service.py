...
class PaymentService:
    ...
    async def get_payment_by_id(self, payment_id: str, session: AsyncSession) -> PaymentSchema | None:
        payment = await self.repo.get_payment_by_id(payment_id=payment_id, session=session)
        return payment

    async def create_base_payment(self, session: AsyncSession, payment_data: PaymentSchema) -> None:
        payment = Payment(
        ...
        try:
            ...
            logger.error(f"Error updating payment status. Current status: {result.status}")
            raise DuplicateException(message=str(exc))
